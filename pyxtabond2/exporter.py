"""
Publication-oriented export of multiple GMM results.

:class:`GMMStargazer` builds comparative regression tables in LaTeX or Word,
similar to Stata's ``esttab`` / ``stargazer`` workflow.
"""
...
def _escape_latex(text) -> str:
    ...
    model name, diagnostic-test name) before inserting it into a raw LaTeX
    table row -- see the identical helper in results.py for why (most
    commonly, an unescaped underscore in a name like ``_cons`` or
    ``L1_Growth`` breaks compilation outside math mode). Only applied on
    the LaTeX path: ``to_word()`` reuses the same strings unescaped, which
    is correct there.
    """
...
class GMMStargazer:
    ...
    Examples
    --------
    >>> from pyxtabond2 import PyXtabond2, GMMStargazer
    >>> 
    ...
    def __init__(self, models, model_names=None):
        ...
        self.ar_lags = self._get_unique_ar_lags()
    ...
    def _get_unique_ar_lags(self):
        """
        Retrieves all Arellano-Bond AR(l) lag orders reported across models
        (models may use different ``artests`` settings), sorted ascending.
        ...
        lags = set()
        for mod in self.models:
            lags.update(mod.diag.get('ar', {}).keys())
        return sorted(lags)
    ...
    def _extract_data(self):
        ...
        data = {
            ...
            "diagnostics": {
                ...
                "Number of groups": [],
                "Instruments": [],
                **{f"AR({lag}) p-value": [] for lag in self.ar_lags},
                "Sargan p-value": [],
                "Hansen p-value": []
            }
        }
        ...
        for mod in self.models:
            ...
            mod_ar = mod.diag.get('ar', {})
            for lag in self.ar_lags:
                p_val = mod_ar[lag][1] if lag in mod_ar else np.nan
                data["diagnostics"][f"AR({lag}) p-value"].append(f"{p_val:.3f}" if not pd.isna(p_val) else "-")
    ...
    def to_latex(self, filepath=None):
        ...
        lines = [
            ...
            "\\toprule",
            " & " + " & ".join([f"\\textbf{{{_escape_latex(name)}}}" for name in self.model_names]) + " \\\\",
            "\\midrule"
        ]

        # Add coefficients and Stats (z/t). Variable names are escaped here
        # (not in results_dict/all_vars themselves, which to_word() also
        # reads and must keep unescaped).
        for var in self.all_vars:
            coef_line = f"{_escape_latex(var)} & " + " & ".join(self.results_dict["coeffs"][var]) + " \\\\"
        ...
        for diag_name, values in self.results_dict["diagnostics"].items():
            lines.append(f"{_escape_latex(diag_name)} & " + " & ".join(values) + " \\\\")
